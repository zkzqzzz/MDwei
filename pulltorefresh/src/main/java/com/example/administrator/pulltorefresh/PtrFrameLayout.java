...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            PLog.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                PLog.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin);
                PLog.d(LOG_TAG, "onMeasure, mCurrentPos: %s, mLastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                PLog.d(LOG_TAG, "onLayout header: %s %s %s %s", left, top, right, bottom);
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                PLog.d(LOG_TAG, "onLayout content: %s %s %s %s", left, top, right, bottom);
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mPtrIndicator.hasLeftStartPosition()) {
                    if (DEBUG) {
                        PLog.d(LOG_TAG, "call onRelease when user release");
                    }
                    onRelease(false);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    PLog.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, mCurrentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown);
                }
    ...
    private void movePos(float deltaY) {
        ...
        if ((deltaY < 0 && mPtrIndicator.isInStartPosition())) {
            if (DEBUG) {
                PLog.e(LOG_TAG, String.format("has reached the top"));
            }
            return;
        }
        ...
        if (mPtrIndicator.willOverTop(to)) {
            if (DEBUG) {
                PLog.e(LOG_TAG, String.format("over top"));
            }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                PLog.i(LOG_TAG, "PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag);
            }
        }
        ...
        if (DEBUG) {
            PLog.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            ...
            public void run() {
                if (DEBUG) {
                    PLog.d(LOG_TAG, "mRefreshCompleteHook resume.");
                }
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (DEBUG) {
                PLog.i(LOG_TAG, "PtrUIHandler: onUIRefreshBegin");
            }
        }
        if (mPtrHandler != null) {
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator.isInStartPosition()) {
            if (mPtrUIHandlerHolder.hasHandler()) {
                ...
                if (DEBUG) {
                    PLog.i(LOG_TAG, "PtrUIHandler: onUIReset");
                }
            }
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                PLog.d(LOG_TAG, "call onRelease after scroll abort");
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                PLog.d(LOG_TAG, "call onRelease after scroll finish");
            }
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        if (DEBUG) {
            PLog.i(LOG_TAG, "refreshComplete");
        }
        ...
        if (delay <= 0) {
            if (DEBUG) {
                PLog.d(LOG_TAG, "performRefreshComplete at once");
            }
            ...
            if (DEBUG) {
                PLog.d(LOG_TAG, "performRefreshComplete after delay: %s", delay);
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                PLog.d(LOG_TAG, "performRefreshComplete do nothing, scrolling: %s, auto refresh: %s",
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mPtrIndicator.hasLeftStartPosition() && !ignoreHook && mRefreshCompleteHook != null) {
            if (DEBUG) {
                PLog.d(LOG_TAG, "notifyUIRefreshComplete mRefreshCompleteHook run.");
            }
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            if (DEBUG) {
                PLog.i(LOG_TAG, "PtrUIHandler: onUIRefreshComplete");
            }
    ...
    private void sendCancelEvent() {
        if (DEBUG) {
            PLog.d(LOG_TAG, "send cancel event");
        }
    ...
    private void sendDownEvent() {
        if (DEBUG) {
            PLog.d(LOG_TAG, "send down event");
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        private void finish() {
            if (DEBUG) {
                PLog.v(LOG_TAG, "finish, mCurrentPos:%s", mPtrIndicator.getCurrentPosY());
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                PLog.d(LOG_TAG, "tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to);
            }
            removeCallbacks(this);
